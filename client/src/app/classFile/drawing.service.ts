...
import { PatchClassMetaData, PatchClass, AddClass } from '../state/actions';

import { AppState, SetSelection } from '../state/app.state';
import { FileState, FileStateModel, filterByEntityType } from '../state/file.state';
import { File, ClassFile, FileEntityType, ClassEntity, RelationEntity, FileEntity, Relation } from '../state/model';
...
export class DrawingService {
  ...
  constructor(protected paperService: PaperService,
    ...
    paperService.hasInitialized.then(() => {
      ...
      this.tool.onMouseDown = (event: paper.ToolEvent) => {
        let action;
        if (event.item) {
          if (event.item.data.type == FileEntityType.Class) {
            action = new SetSelection(paperService.fileId, [event.item.data.id]);
          } else if (event.item.data.type == FileEntityType.Relation) {
            action = new SetSelection(paperService.fileId, [event.item.data.id]);
          } else {
      ...
      this.tool.onMouseDrag = (event: paper.ToolEvent) => {
        ...
        if (selEntity == null) {
          return;
        } else if (selEntity.type == FileEntityType.Class) {
  ...
  public draw(data: File) {
    ...
    if (data) {
      ...
      //draw the classes
      filterByEntityType(FileEntityType.Class, data).forEach((clas: ClassEntity) => {
        this.drawClass(clas);
      });
      //if we have relations, draw those too
      filterByEntityType(FileEntityType.Relation, data).forEach((rel: RelationEntity) => {
